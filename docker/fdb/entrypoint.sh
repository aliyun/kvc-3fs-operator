#!/bin/bash
# This script is used to run the entrypoint.sh script inside the docker container
...
function create_server_environment() {
    ...
    if [[ "$FDB_NETWORKING_MODE" == "host" ]]; then
        ...
        public_ip=$(hostname -i | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -n 1)
    else
...
sed -i "s|\${PUBLIC_IP}|${PUBLIC_IP}|g" /etc/foundationdb/foundationdb.conf
sed -i "s|\${MACHINE_ID}|$(hostname)|g" /etc/foundationdb/foundationdb.conf
